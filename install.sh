...
install_theme() {
  ...
  wget -q "$THEME_URL" -O "/root/${THEME_NAME}.zip" || { echo -e "${RED}Download gagal.${NC}"; return; }

  echo "[+] Mengekstrak theme..."
  sudo unzip -o "/root/${THEME_NAME}.zip" -d /root/pterodactyl || { echo -e "${RED}Unzip gagal.${NC}"; return; }
